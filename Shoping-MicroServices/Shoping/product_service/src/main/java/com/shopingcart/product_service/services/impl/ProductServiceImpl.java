...
import com.shopingcart.product_service.errors.CustomError;
...
public class ProductServiceImpl implements ProductService {
    ...
    public ProductDTO getProductById(long prodId) {
        ...
        if(product == null){
            throw new CustomError("Table are not ready","DATABSE_ISSUE");
        }
        ProductDTO productDto = new ProductDTO();
        ...
        throw new CustomError("Product with given id " + prodId + " doesn't exists.", "PRODUCT_NOT_FOUND");
    ...
    public ProductDTO updateProduct(long prodId, Product product) {
        ...
        if (isExists.isPresent()) {
            ...
            if (isExists.get().getProdQuantity() != product.getProdQuantity() && product.getProdQuantity() > 0) {
                isExists.get().setProdQuantity(product.getProdQuantity());
            }
    ...
    public boolean outUpdateProduct(long prodId, long outQuantity) {
        ...
        if (outQuantity > isExists.get().getProdQuantity()) {
            throw new CustomError("Quantity is less than you want order" + isExists.get().getProdQuantity(), "INSUFFIENT_QUENTITY");
        }
